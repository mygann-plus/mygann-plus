...
const domQuery = {
  ...
  dialogBody: () => document.querySelector('.modal-body'),
  courseTitles: () => document.querySelectorAll('#coursesContainer h3'),
};

const formatDate = dateString => { // remove time and year
...
async function getFirstSemesterAssignments(firstSemCourses, firstSemMarkingPeriod) {
  await waitForLoad(domQuery.assignmentTable);
  const title = domQuery.dialogHeader().textContent;
  const courseId = Array.from(domQuery.courseTitles())
    .find(e => e.textContent === title)
    .closest('.row')
    .querySelector('.btn[data-analysis')
    .dataset.analysis - 1;
...
function generateAssignmentSectionTable(sectionName, assignments) {
...
async function showFirstSemesterGrades(firstSemCourses, firstSemMarkingPeriodId, modalBody) {
  const assignments = await getFirstSemesterAssignments(firstSemCourses, firstSemMarkingPeriodId);
  const assignmentSections = {};
  for (const assignment of assignments) {
    const { AssignmentType: type } = assignment;
...
function handleShowFirstSemesterClick(e, firstSemesterCourseList, markingPeriodId, modalBody) {
...
async function insertFirstSemesterButton(firstSemesterCourseList, markingPeriodId, nonacademicClasses) {
  ...
  const firstSemesterBtn = constructButton(
    ...
    e => handleShowFirstSemesterClick(e, firstSemesterCourseList, markingPeriodId, modalBody),
    '',
  );

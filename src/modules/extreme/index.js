...
function incrementCloseButton(button) {
...
function hideProcrastinationFrame(e) {
  document.querySelector(`.${selectors.frame}`).remove();
  document.querySelector(`.${selectors.title}`).remove();
  e.target.closest('button').remove();
  document.querySelector('#assignment-center-list-view').style.display = '';
}
...
function showProcrastinationFrame() {
  const existingFrame = document.querySelector(`.${selectors.frame}`);
  let existingUrl = '';
  if (existingFrame) {
  ...
  const assignmentCenter = document.querySelector('#assignment-center-list-view');

  const existingClose = document.querySelector(`.${selectors.close}`);
  if (existingClose) {

...
function toggleReleaseNotes(e: Event) {
  e.preventDefault();
  document
    .querySelector(`.${selectors.wrap}`)
    .classList.toggle(selectors.releaseNotesShown);
}
async function insertReleaseNotes(dialogBody: HTMLElement) {
  const releaseNotesWrap = dialogBody.querySelector(
    `#${selectors.releaseNotes}`,
  );
  ...
  if (!releases.length) {
    ...
    releaseNotesWrap.innerHTML = await marked(
      releases.map((r) => r.body).join('\n\n'),
    ); // parse markdown
  }
}

function showUpdateFlyout(aboutBody: HTMLElement) {
  const releaseNotesLink = aboutBody.querySelector(
    `#${selectors.releaseNotesLink}`,
  ) as HTMLElement;

  const flyout = new Flyout(
    (
      <span>
        New version of MyGann+ Check out what&apos;s new!
        <br />
  ...
  return (
    <div className={selectors.wrap}>
      ...
      <p>
        ...
        <a
          href="#"
          id={selectors.releaseNotesLink}
          onClick={(e: any) => toggleReleaseNotes(e)}
        >
          what&apos;s new
        </a>
        )
      ...
      <p>
        <b>Created By:</b> Matan Kotler-Berkowitz
      </p>
      <p>
      ...
      <ul>
        <li>
          ...
          , then{' '}
          <a href="#" onClick={(e: any) => handleBugReportClick(e)}>
            submit a bug report
          </a>
          .{createBugReportUi()}
        </li>
      ...
      <ul>
        ...
        <li>
          Shai Mann-Robison and Micah Shire-Plumb for beta testing and
          suggestions
        </li>
      </ul>
    </div>
...
async function aboutMain() {
  const desktopMenuLink = appendUserMenuButton(
    'About MyGann+',
    showDialog,
  ).desktop;
  ...
  if (await hasUpdated()) {
    const avatar = getHeader().parentNode.querySelector(
      '.bb-avatar-wrapper-nav',
    );
    const avatarBadge = (
      <span
        className={selectors.updateBadge}
        id={selectors.desktopAvatarBadge}
      />
    );
    avatar.after(avatarBadge);

    const linkBadge = (
      <span className={selectors.updateBadge} id={selectors.desktopLinkBadge} />
    );
